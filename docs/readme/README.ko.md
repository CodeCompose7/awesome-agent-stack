...
세 컬렉션 모두 프런트매터는 [`packages/core/src/content.ts`](../../packages/core/src/content.ts)의
...
[`@awesome-ai-stack/core`](../../packages/core/) 테마 패키지에 있고, 각 카탈로그
...
```text
packages/core/                  # @awesome-ai-stack/core — 테마(엔진)
...
- **UI 문자열**은 [`src/i18n/ui.ts`](../../packages/core/src/i18n/ui.ts)의 `ui` 사전에 있습니다.
...
그러면 상세 페이지에 **구현** 탭
([`ProjectViewer`](../../packages/core/src/components/ProjectViewer.astro))이 뜨고, 프로젝트마다
[`src/lib/project.ts`](../../packages/core/src/lib/project.ts)가 빌드 시점에:
...
- **Mermaid** 다이어그램은 개요 본문(```mermaid 펜스 블록)과 샘플별 `diagram:`
  ...
  바뀝니다([`MermaidLoader.astro`](../../packages/core/src/components/MermaidLoader.astro)).
...
[`src/lib/github.ts`](../../packages/core/src/lib/github.ts)에 있습니다. 레포별로 메모이즈되고
...
있습니다. 테마 마크다운 파이프라인([`packages/core/markdown.mjs`](../../packages/core/markdown.mjs))의 `remarkGlossary` 플러그인이
...
- **백링크**(도구 → 글): Astro에는 **네이티브 백링크가 없어서**,
  [`getArticlesForTool()`](../../packages/core/src/lib/articles.ts)이 빌드 시점에 역방향 조회를
...
스키마([`packages/core/src/content.ts`](../../packages/core/src/content.ts))는 필요할 때 더
