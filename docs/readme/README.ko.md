...
| 모노레포    | pnpm 워크스페이스 — 테마 패키지 하나 + N개 사이트    |
...
세 컬렉션 모두 프런트매터는 [`packages/stack-site-builder/src/content.ts`](../../packages/stack-site-builder/src/content.ts)의
...
이 저장소는 **pnpm 워크스페이스**입니다. 사이트와 무관한 모든 것 — 라우트,
컴포넌트, 스타일, 마크다운 파이프라인, 콘텐츠 스키마 — 은
[`stack-site-builder`](../../packages/stack-site-builder/) 테마 패키지에 있고, 각 카탈로그
사이트는 콘텐츠·분류 데이터·작은 설정만 가진 얇은 소비자로 `sites/` 아래에
둡니다. [`sites/awesome-ai-stack/`](../../sites/awesome-ai-stack/)이 이 사이트이며,
카탈로그를 더 만들 때(예: AI 이미지 스택)도 `sites/` 아래에 얇은 폴더 하나를
추가하는 방식은 같습니다. 최종 목표는 테마를 npm에 배포하고 사이트를 저장소별로
분리하는 것이며, 이 워크스페이스는 테마 경계를 먼저 안정화하는 작업대입니다.

루트 스크립트는 메인 사이트를 대상으로 하고(`pnpm dev` / `pnpm build`), 전체
빌드는 `pnpm -r build`를 씁니다.

## 프로젝트 구조

```text
packages/stack-site-builder/                  # stack-site-builder — 테마(엔진)
├─ index.mjs                    # 통합: 모든 라우트 주입, 마크다운/테일윈드 연결,
│                               #   @aas-data/*·@assets/*를 소비 사이트로 별칭
├─ markdown.mjs                 # remark/rehype 파이프라인 (mermaid, 슬라이드, [[위키링크]]…)
├─ dev/                         # 개발 전용 local-samples 미들웨어 (실행 마법사)
└─ src/
   ├─ content.ts                # 공유 Zod 스키마 — defineAasCollections()
   ├─ components/               # 카드·상세 페이지·탭·슬라이드·뷰어 등 45개 안팎의 컴포넌트
   ├─ layouts/BaseLayout.astro  # 공용 셸 (헤더/푸터/테마 + 언어 전환)
   ├─ lib/                      # 컬렉션 조회, 백링크, GitHub 스타, 샘플 렌더링…
   ├─ i18n/ui.ts                # 언어, UI 문자열, t() 번역기 (사이트별 오버라이드 지원)
   ├─ pages/                    # 모든 라우트 (루트 EN + /ko 미러) — 사이트에 주입됨
   └─ styles/global.css         # Tailwind + 마크다운 prose + 슬라이드 스타일

sites/awesome-ai-stack/                 # 이 사이트 — 콘텐츠와 설정만
├─ astro.config.mjs             # site/base/i18n + aasTheme({ glossary })
├─ src/content.config.ts        # 호출 한 번: defineAasCollections({ categoryMap })
├─ src/content/
...
아래에서 서빙되며, [`astro.config.mjs`](../../sites/awesome-ai-stack/astro.config.mjs)의 Astro `i18n`으로
설정합니다. 헤더의 언어 토글이 같은 페이지의 다른 로케일로 연결됩니다.

- **UI 문자열**은 [`src/i18n/ui.ts`](../../packages/stack-site-builder/src/i18n/ui.ts)의 `ui` 사전에 있습니다.
- **카테고리 라벨**은 [`src/data/categories.ts`](../../sites/awesome-ai-stack/src/data/categories.ts)에
...
[`content/stacks/{en,ko}/langgraph.mdx`](../../sites/awesome-ai-stack/src/content/stacks/en/langgraph.mdx)가
실제 예시입니다.
...
[`samples/langgraph_1/`](../../sites/awesome-ai-stack/samples/langgraph_1/)). 프런트매터에 나열합니다 —
...
그러면 상세 페이지에 **구현** 탭
([`ProjectViewer`](../../packages/stack-site-builder/src/components/ProjectViewer.astro))이 뜨고, 프로젝트마다
[`src/lib/project.ts`](../../packages/stack-site-builder/src/lib/project.ts)가 빌드 시점에:
...
- **Mermaid** 다이어그램은 개요 본문(```mermaid 펜스 블록)과 샘플별 `diagram:`
  ...
  바뀝니다([`MermaidLoader.astro`](../../packages/stack-site-builder/src/components/MermaidLoader.astro)).
...
[`src/lib/github.ts`](../../packages/stack-site-builder/src/lib/github.ts)에 있습니다. 레포별로 메모이즈되고
...
있습니다. 테마 마크다운 파이프라인([`packages/stack-site-builder/markdown.mjs`](../../packages/stack-site-builder/markdown.mjs))의 `remarkGlossary` 플러그인이
각 마커를 중앙 용어집([`src/data/glossary.mjs`](../../sites/awesome-ai-stack/src/data/glossary.mjs))에
대조해 빌드 시점에 해석합니다:
...
- **백링크**(도구 → 글): Astro에는 **네이티브 백링크가 없어서**,
  [`getArticlesForTool()`](../../packages/stack-site-builder/src/lib/articles.ts)이 빌드 시점에 역방향 조회를
...
로케일당 MDX 파일 하나를 `sites/awesome-ai-stack/src/content/stacks/en/`와 `sites/awesome-ai-stack/src/content/stacks/ko/`에
...
스키마([`packages/stack-site-builder/src/content.ts`](../../packages/stack-site-builder/src/content.ts))는 필요할 때 더
많은 것을 지원합니다: `formerNames`, `pricingTiers`/`pricingNote`/`pricingSource`,
`related` 도구, `deprecated`, `docVersion`/`updated`, 그리고 위에서 설명한
`samples`/`projects` 필드.

**새 카테고리**를 추가하려면 [`src/data/categories.ts`](../../sites/awesome-ai-stack/src/data/categories.ts)의
...
맞춰 [`astro.config.mjs`](../../sites/awesome-ai-stack/astro.config.mjs)에 설정돼 있습니다. 커스텀 도메인을
