...
둡니다. [`sites/ai-stack/`](../../sites/ai-stack/)이 이 사이트이고,
[`sites/ai-image-stack/`](../../sites/ai-image-stack/)은 같은 테마 위에 올린 두
번째 카탈로그입니다. 최종 목표는 테마를 npm에 배포하고 사이트를 저장소별로
분리하는 것이며, 이 워크스페이스는 테마 경계를 먼저 안정화하는 작업대입니다.

루트 스크립트는 메인 사이트를 대상으로 합니다(`pnpm dev` / `pnpm build`). 다른
사이트는 `pnpm --filter ai-image-stack dev`, 전체 빌드는 `pnpm -r build`를 씁니다.
...
sites/ai-image-stack/           # 두 번째 카탈로그 사이트 — 같은 구조, 콘텐츠만 다름
```

## 국제화 (EN / KO)

사이트는 이중 언어입니다. `en`이 기본 로케일(루트에서 서빙)이고 `ko`는 `/ko/`
아래에서 서빙되며, [`astro.config.mjs`](../../sites/ai-stack/astro.config.mjs)의 Astro `i18n`으로
설정합니다. 헤더의 언어 토글이 같은 페이지의 다른 로케일로 연결됩니다.

- **UI 문자열**은 [`src/i18n/ui.ts`](../../packages/stack-site-builder/src/i18n/ui.ts)의 `ui` 사전에 있습니다.
- **카테고리 라벨**은 [`src/data/categories.ts`](../../sites/ai-stack/src/data/categories.ts)에
...
[`content/stacks/{en,ko}/langgraph.mdx`](../../sites/ai-stack/src/content/stacks/en/langgraph.mdx)가
실제 예시입니다.
...
[`samples/langgraph_1/`](../../sites/ai-stack/samples/langgraph_1/)). 프런트매터에 나열합니다 —
...
각 마커를 중앙 용어집([`src/data/glossary.mjs`](../../sites/ai-stack/src/data/glossary.mjs))에
대조해 빌드 시점에 해석합니다:
...
로케일당 MDX 파일 하나를 `sites/ai-stack/src/content/stacks/en/`와 `sites/ai-stack/src/content/stacks/ko/`에
...
**새 카테고리**를 추가하려면 [`src/data/categories.ts`](../../sites/ai-stack/src/data/categories.ts)의
...
맞춰 [`astro.config.mjs`](../../sites/ai-stack/astro.config.mjs)에 설정돼 있습니다. 커스텀 도메인을
